...
def generate_random_board():
    ...
    # Pattern for a baseline valid Sudoku grid
    ...
    squares = side * side
    empties = random.randint(30, 40)  # Number of empty squares
...
class Grid:
    def __init__(self, rows, cols, width, height, win):
        self.board = generate_random_board()  # Use the random board generator
        self.rows = rows
        self.cols = cols
...
def redraw_window(win, board, time, strikes):
    ...
    win.blit(text, (490 - 110, 550))  # Move timer to the top-left corner
    text = fnt.render("X " * strikes, 1, (255, 0, 0))
    win.blit(text, (10, 550))  # Display strikes next to the timer
    board.draw()
